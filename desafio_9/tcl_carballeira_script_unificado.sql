DROP DATABASE IF EXISTS book_dep1;
...
DROP TABLE IF EXISTS Pais_Envios;
...
USE book_dep1; -- Seleccionar la base de datos

DELIMITER //

-- Crear el procedimiento almacenado para insertar registros en la tabla Libros con savepoints
CREATE PROCEDURE InsertarRegistrosConSavepoints()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK TO InitialSavepoint; -- En caso de error, deshacer hasta el punto de inicio
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error al insertar registros.';
    END;

    START TRANSACTION; -- Iniciar transacción
    SAVEPOINT InitialSavepoint; -- Establecer savepoint inicial

    -- Insertar registros en la tabla Libros
    INSERT INTO Libros (titulo, ID_AUTOR, ID_EDITORIAL, anio_publicacion, precio, cantidad, ID_CATEGORIA)
    VALUES
        ('El nuevo libro', 11, 4, '2023', 30.00, 10, 1),
        ('Aventuras mágicas', 12, 5, '2022', 25.50, 15, 3),
        ('En busca del tesoro', 13, 6, '2021', 20.75, 20, 2),
        ('El misterio revelado', 14, 7, '2023', 18.90, 18, 4),
        ('El viaje intergaláctico', 15, 8, '2022', 28.00, 12, 5),
        ('La gran odisea', 16, 9, '2021', 22.50, 25, 1),
        ('Romance en París', 17, 10, '2020', 24.25, 30, 6),
        ('Cuentos de hadas modernos', 11, 4, '2023', 26.80, 22, 7);

    SAVEPOINT SavepointAfter4; -- Establecer savepoint después de los primeros 4 registros

    -- Insertar más registros en la tabla Libros
    INSERT INTO Libros (titulo, ID_AUTOR, ID_EDITORIAL, anio_publicacion, precio, cantidad, ID_CATEGORIA)
    VALUES
        ('Secretos en la mansión', 12, 5, '2022', 21.00, 18, 2),
        ('Amanecer sangriento', 13, 6, '2021', 19.75, 15, 8);

    SAVEPOINT SavepointAfter8; -- Establecer savepoint después de todos los registros

    -- DELETE FROM Libros WHERE ID_LIBRO BETWEEN 1 AND 8; -- Comentario: Elimina los registros del 1 al 8
    -- COMMIT; -- Comentario: Realizar el COMMIT para confirmar los cambios
END //
...
-- Crear el procedimiento almacenado para procesar registros en la tabla Pedidos
...
BEGIN
    ...
    ELSE
        ...
        VALUES 
            (1, '2023-08-01', 180.50),
            (2, '2023-08-02', 210.25);
        SELECT "Se insertaron 2 registros en la tabla Pedidos." AS Message;
    END IF;
